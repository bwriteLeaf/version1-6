...
def pic3_1(figGen):
    ...
    list_a1 = ['blood_sick_A =1', 'favism_A =1', 'congenital_heart_disease_A=1',
               'infant_mortality_A=1','(hearing_disorder_A=1 or seeing_disorder_A=1)','dysgnosia_A=1']
    ...
    list_d1 = ['blood_sick_A  is not null', 'favism_A  is not null', 'congenital_heart_disease_A is not null',
               'infant_mortality_A is not null','(hearing_disorder_A is not null or seeing_disorder_A is not null)','dysgnosia_A is not null']
    ...
    list_d = [list_d1, list_d2]
    labels = ['女方', '男方']
    xlabels = ['地中海贫血', 'G6PD缺乏症', "先天性心脏病", '新生儿或婴幼儿死亡史', "内生障碍", '先天性智力低下']
...
def pic3_6(figGen):
    list_a = ['(meategg_A=0)', '(hate_veg_A=1)', '(raw_meat_A=1)']
    list_d = ['', '', '', '']
    ...
    list_a = ['(bmi_A < 18.5)', '(bmi_A >= 24 and bmi_A < 28)', '(bmi_A >= 28)']
    list_d = ['', '', '', '']
    labels = ["低体重", '超重', "肥胖"]
    ...
    list_a = ["(bmi_A >= 28)", "(bmi_A >= 24 and bmi < 28)", "(bmi_A >= 18.5 and bmi < 24)", "(bmi_A < 18.5)"]
    list_d = ["", "", "", ""]
    labels = ["肥胖","偏重","正常","偏瘦"]
    ...
    list_a = ["(anemia_A=1)", "(bhb<110)"]
    list_d = ["(is_ill_A is not null and bhb<110)",'']
    labels = ["自报患病率", "检出率"]
