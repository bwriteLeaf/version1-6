...
class DBInterfaceDraw:
    ...
    def getDistrictData(self, attrExpr, divideExpr, diseaseName, dbName, districtType, year, isPercent=True,
                        complete=True):
        ret_list = []
        ...
        for i in range(0, len(district_list)):
            ...
            # sql分别执行
            r_up_s, sql = self.execSQL(code_district, attrExpr, dbName, year, complete)
            r_up = int(r_up_s)
            if isPercent:
                r_down_s, sql = self.execSQL(code_district, divideExpr, dbName, year, complete)
                r_down = int(r_down_s)
    ...
    def getDiffDistrictData(self, attrExprList, divideExprList, diseaseNameList, dbName, districtType, year,
        ...
        for j in range(0, len(attrExprList)):
            ...
            ret_list = self.getDistrictData(attrExpr, divideExpr, diseaseName, dbName, districtType, year, isPercent,
                                            complete)
    ...
    def getTopk(self, attrExprList, divideExprList, diseaseNameList, dbName, mainType, retType, k, year, sortType=True,
                isPercent=True, complete=True):
        ret_list = []
        sorted_list = []

        if mainType == "disease":
            data_raw = self.getDiffDistrictData(attrExprList, divideExprList, diseaseNameList, dbName, "all", year,
                                                isPercent, complete)
            ret_list = [x[0] for x in data_raw]
        if mainType == "district":
            data_raw = self.getDiffDistrictData(attrExprList, divideExprList, diseaseNameList, dbName, "district", year,
                                                isPercent, complete)
            ret_list = data_raw[0]
    ...
    def textSubstitute(self, attrExprList, divideExprList, diseaseNameList, dbName, mainType, textList, dimenType,
                       complete):
        sub = 0
        if mainType == "disease":
            data_raw = self.getDiffDistrictData(attrExprList, divideExprList, diseaseNameList, dbName, "all",
                                                self.__conf.year,
                                                isPercent=True, complete=True)
            ret_list = [x[0] for x in data_raw]
        ...
        if mainType == "year":
            this_list, last_list = self.getTwoYearByDistrict(attrExprList[0], divideExprList[0], dbName, "all",
                                                             complete)
